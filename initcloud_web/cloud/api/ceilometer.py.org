...
class Alarm(base.APIResourceWrapper):
    ...
    _attrs = ['alarm_actions', 'ok_actions', 'name', 'timestamp', 'description',
              'time_constraints', 'enabled', 'state_timestamp','threshold_rule',
              'alarm_id', 'state', 'insufficient_data_actions', 'repeat_actions',
              'user_id', 'project_id', 'type', 'severity']

#--------------------------------------------------------------------------------

#------------------------- alarm -------------------------------------------------
def make_threshold_rule(meter_name = None, comparison_operator = None, threshold =None, **kwargs):
    threshold_rule_attr = ['evaluation_periods', 'period','statistic','query','exclude_outliers']
    threshold_rule = dict((key, value) for (key, value) in kwargs.items() if (key in threshold_rule_attr))
    if threshold is not None:
        try:
            threshold_rule['threshold'] = int(threshold)
        except:
            threshold_rule['threshold'] = 0.0
...
def alarm_create(request, name, threshold_rule, severity, alarm_actions, user_id = None, project_id = None):
    alarm = ceilometerclient(request).alarms.create(name = name, threshold_rule = threshold_rule, severity = severity, alarm_actions = alarm_actions, user_id = user_id , project_id = project_id)
    return alarm

def alarm_delete(request, alarm_id):
    return ceilometerclient(request).alarms.delete(alarm_id)

def alarm_update_query(request, alarm_id, name, threshold_rule, user_id = None, project_id = None):
    return ceilometerclient(request).alarms.update(alarm_id, name = name, threshold_rule = threshold_rule, user_id = user_id, project_id = project_id)
#--------------------------------------------------------------------------------------
