...
class Network(models.Model):
    ...
    name = models.CharField(_('Network Name'), null=False, blank=False,
                max_length=128)
    network_id = models.CharField(_('OS Network UUID'), null=True,
                blank=True, max_length=128)
    status = models.IntegerField(_("Status"), null=False,
                blank=False, choices=NETWORK_STATES, default=0)
    ...
    def __unicode__(self):
       return u"<Network ID:%s Name:%s>" % (self.id, self.name) 
...
class Subnet(models.Model):
    ...
    def __unicode__(self):
       return u"<Subnet ID:%s CIDR:%s>" % (self.id, self.address) 
...
class Router(LivingDeadModel):
    ...
    gateway = models.CharField(_('Gateway IP Address'), null=True, blank=True, max_length=128)
    is_gateway = models.BooleanField(_("Has Gateway Setted"), null=False, blank=False, default=False)
    ...
    def __unicode__(self):
       return u"<Router ID:%s Name:%s Gateway:%s>" % (
                    self.id, self.name, self.gateway) 
    ...
    def set_gateway_failed(self):
        self.gateway = None
        self.is_gateway = False
        self.status = NETWORK_STATE_ACTIVE
        self.save()
