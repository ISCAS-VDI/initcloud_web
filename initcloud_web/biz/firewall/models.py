...
class Firewall(models.Model):
    ...
    def is_in_use(self):
        ...
        return Instance.objects.filter(deleted=False,
                                       firewall_group=self).exists()

    def __unicode__(self):
        return u"<Firewall ID:%s Name:%s>" % (self.id, self.name)
...
class FirewallRules(models.Model):
    ...
    def __unicode__(self):
        return u"<FirewallRule ID:%s Port:%s~%s>" % (
                self.id, self.port_range_min, self.port_range_max)
